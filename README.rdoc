...
To use the client, you must first authorize the client with the API by initializing an instance of the Pipejump::Session class. You will later use this instance for communication with the API.


     @session = Pipejump::Session.new(:email => 'you@email.com', :password => 'your_password')


If the credentials are incorrect, this will raise a Pipejump::AuthorizationFailed error.

If authorization succeeds, this will return a new Pipejump::Session instance which you can use to communicate with the API.
